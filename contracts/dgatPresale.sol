...
contract preSaleDgat{
    ...
    constructor(address payable _owner, IBEP20 _token) {
        owner = _owner;  // 0xc19bfd81b9c1e21d7d69f13e4147827b25e95d96
        token = _token;  // 0x96E3BD1915483eD6E6569e908a0F6F49434557eD
        priceFeedBnb = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE);
        tokenPerUsd = 3333333;
        minAmount = 0.2 ether;
        maxAmount = 5 ether;
        maxSell = token.totalSupply().mul(12).div(100);
        preSaleStartTime = block.timestamp.add(20 minutes);
        preSaleEndTime = preSaleStartTime.add(14 days); 
        referralPercent = 10;
    ...
    function buyTokenDgat(address _referrer) payable public {
        ...
        if(whiteList[msg.sender] || whiteList[_referrer]){
            if(msg.value >= minAmount && msg.value < 0.5 ether){
                balances[msg.sender] = balances[msg.sender].add(numberOfTokens.add(numberOfTokens.mul(whiteListBonus[0]).div(100)));
                token.transferFrom(owner, address(this), numberOfTokens.add(numberOfTokens.mul(whiteListBonus[0]).div(100)));
            }else if(msg.value >= 0.5 ether && msg.value < 1 ether){
                balances[msg.sender] = balances[msg.sender].add(numberOfTokens.add(numberOfTokens.mul(whiteListBonus[1]).div(100)));
                token.transferFrom(owner, address(this), numberOfTokens.add(numberOfTokens.mul(whiteListBonus[1]).div(100)));
            }else if(msg.value >= 1 ether && msg.value < 3 ether){
                balances[msg.sender] = balances[msg.sender].add(numberOfTokens.add(numberOfTokens.mul(whiteListBonus[2]).div(100)));
                token.transferFrom(owner, address(this), numberOfTokens.add(numberOfTokens.mul(whiteListBonus[2]).div(100)));
            }else if(msg.value >= 3 ether && msg.value < 5 ether){
                balances[msg.sender] = balances[msg.sender].add(numberOfTokens.add(numberOfTokens.mul(whiteListBonus[3]).div(100)));
                token.transferFrom(owner, address(this), numberOfTokens.add(numberOfTokens.mul(whiteListBonus[3]).div(100)));
            }else{
                balances[msg.sender] = balances[msg.sender].add(numberOfTokens.add(numberOfTokens.mul(whiteListBonus[4]).div(100)));
                token.transferFrom(owner, address(this), numberOfTokens.add(numberOfTokens.mul(whiteListBonus[4]).div(100)));
            }
        }else {
            if(msg.value >= minAmount && msg.value < 0.5 ether){
    ...
    function transferFunds(address payable _preSaleWallet, uint256 _value) external onlyOwner{
        _preSaleWallet.transfer(_value);
    }
